"""GPT-5 evaluation module with custom API configuration
"""
import os
import torch
import time
...
class GPT5():
    ...
    def __init__(self, model_path: str = "gpt-5") -> None:
        """GPT-5 model wrapper with custom API configuration

        Args:
            model_path (str): model name
        """
        self.model_path = model_path
        # Set custom API configuration for GPT-5
        ...
        if self.support_multi_image:
            ...
            for message in inputs:
                if message["type"] == "image":
                    # Convert image to proper data URL format for GPT-5 API
