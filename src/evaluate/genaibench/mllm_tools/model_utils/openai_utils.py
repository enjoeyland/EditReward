...
def _requires_chatml(model: str) -> bool:
    ...
    return "turbo" in model.lower() or "gpt-4" in model.lower() or "gpt-5" in model.lower() or "chatgpt" in model.lower() or "gemini-2.5-pro" in model.lower() or "gemini-2.5-flash" in model.lower() or "gemini-1.5-pro" in model.lower()
...
def local_image_to_data_url(image:Union[str, Image.Image, Path]) -> str:
    if isinstance(image, Path) and image.exists() or isinstance(image, str) and os.path.exists(image):
        ...
        # Ensure image has a format, default to PNG if None
        image_format = image.format if image.format else 'PNG'
        dummy_path = f"temp.{image_format.lower()}"
        mime_type, _ = guess_type(dummy_path)
        if mime_type is None:
            mime_type = 'image/png'  # Default to PNG MIME type
        # encode the image
