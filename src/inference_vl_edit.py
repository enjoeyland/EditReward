...
from .train_qwen_vl_edit import create_model_and_processor
from .model.qwen_vl_trainer import is_qwen3_vl
...
class EditRewardVLInferencer:
    """
    Unified inferencer for Qwen2.5-VL and Qwen3-VL.
    It selects preprocessing path based on model_name_or_path in config.
    """
    ...
    def __init__(
        ...
        self._is_qwen3 = is_qwen3_vl(model_config.model_name_or_path)
        ...
        if os.path.exists(full_ckpt):
            ...
            state_dict = safetensors.torch.load_file(
                full_ckpt_safetensors, device=self.device
            )
        else:
    ...
    def prepare_batch(self, image_src, image_paths, prompts):
        ...
        def _build_batch_qwen25(prompts, image_src, image_paths, reward_dim):
        ...
        def _build_batch_qwen3(prompts, image_src, image_paths, reward_dim):
            messages = _build_messages(prompts, image_src, image_paths, reward_dim)
        ...
        build_batch = _build_batch_qwen3 if self._is_qwen3 else _build_batch_qwen25

        if self.rm_head_type == "ranknet_multi_head":
            batch_dim1 = build_batch(prompts, image_src, image_paths, reward_dim="dim1")
            batch_dim2 = build_batch(prompts, image_src, image_paths, reward_dim="dim2")
            return {"batch_dim1": batch_dim1, "batch_dim2": batch_dim2}

        batch = build_batch(prompts, image_src, image_paths, reward_dim=self.reward_dim)
        return {"batch": batch}
